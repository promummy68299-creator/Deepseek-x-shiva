...
class Database:
    ...
    def init_tables(self):
        with self.get_connection() as conn:
            ...
            default_plans = [
                {"id": 1, "price": 99, "validity": 15, "label": "1 Credit - 15 Days"},
                {"id": 2, "price": 199, "validity": 30, "label": "1 Credit - 30 Days"}
            ]
    ...
    def get_plans(self) -> List[Dict]:
        try:
            plans_json = self.get_setting('plans') or '[]'
    ...
    def add_payment(self, user_id: int, amount: int, plan_json: str, screenshot_path: str, utr: str = ""):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO payments (user_id, amount, plan_json, screenshot_path, utr)
                VALUES (?, ?, ?, ?, ?)
            ''', (user_id, amount, plan_json, screenshot_path, utr))
            conn.commit()
    ...
    def approve_payment(self, payment_id: int) -> bool:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT user_id, plan_json FROM payments WHERE id = ? AND status = "pending"', (payment_id,))
            ...
            expiry_date = datetime.now() + timedelta(days=validity)
            cursor.execute('''
                INSERT INTO credits (user_id, credit_count, plan_price, plan_validity, purchase_date, expiry_date)
                VALUES (?, 1, ?, ?, CURRENT_TIMESTAMP, ?)
            ''', (payment['user_id'], plan.get('price', 99), validity, expiry_date.isoformat()))
            conn.commit()
            return True
    ...
    def add_credits_manual(self, user_id: int, count: int, plan_price: int = 0, plan_validity: int = 15):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            expiry_date = datetime.now() + timedelta(days=plan_validity)
            for _ in range(count):
                cursor.execute('''
                    INSERT INTO credits (user_id, credit_count, plan_price, plan_validity, purchase_date, expiry_date)
                    VALUES (?, 1, ?, ?, CURRENT_TIMESTAMP, ?)
                ''', (user_id, plan_price, plan_validity, expiry_date.isoformat()))
            conn.commit()
            return True
    ...
    def get_credit_details(self, user_id: int) -> Optional[Dict]:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT plan_price, plan_validity, expiry_date FROM credits 
                WHERE user_id = ? AND is_active = 1 AND expiry_date > datetime('now')
                ORDER BY expiry_date ASC
                LIMIT 1
            ''', (user_id,))
...
class DeploymentManager:
    ...
    def validate_bot_token(self, token: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            logging.error("Token validation timed out")
            return False, None
    ...
    def deploy_bot(self, user_id: int, bot_token: str) -> Tuple[bool, str]:
        ...
        if not is_valid:
            return False, "❌ Invalid or expired bot token. Please check and try again."
        ...
        # Prevent duplicate running deployments (optional)
        ...
        if not credit_id:
            return False, "❌ Could not use a credit. Please try again."
        
        # Get template
        template_content = self.get_template_content()
        if not template_content:
            # Revert credit? Not needed for now.
            return False, "❌ No deployment template available. Contact admin."
        ...
        if success:
            ...
            try:
                ...
                pass
            return False, f"❌ Bot failed to start. Check logs for details.\n\n📋 Error:\n```\n{error_log}\n```"
    ...
    def check_expired_credits(self):
        ...
        for credit in expired:
            ...
            for dep in deployments:
                ...
                self.db.add_log(user_id, 'credit_expired', f'Bot @{dep["bot_username"]} stopped')
            
            user = self.db.get_user(user_id)
            if user:
                try:
                    msg = f"⏰ **Credit Expired!**\n\nYour credit has expired on {credit['expiry_date'][:10]}.\n"
                    if deployments:
                        msg += f"\n🤖 **Bots Stopped:**\n"
                        for dep in deployments:
...
'''
    ...
    def setup_handlers(self):
        ...
        def buy_credits(message):
            ...
            if not plans:
                self.bot.send_message(message.chat.id, "❌ No plans available. Contact admin.", parse_mode='Markdown')
                return
            
            markup = types.InlineKeyboardMarkup(row_width=1)
            for plan in plans:
                label = f"🛒 {plan['label']} - ₹{plan['price']}"
                markup.add(types.InlineKeyboardButton(label, callback_data=f"buy_plan_{plan['id']}"))
        ...
        @self.bot.callback_query_handler(func=lambda call: call.data.startswith('buy_plan_'))
        def buy_plan_callback(call):
            ...
            for p in plans:
                if p['id'] == plan_id:
                    ...
                    break
            if not plan:
                self.bot.answer_callback_query(call.id, "❌ Plan not found!", show_alert=True)
                return
            ...
            qr_io = QRGenerator.generate(upi_id, plan['price'], upi_name)
            ...
            self.bot.send_photo(
                ...
                caption=f"💳 **Payment Details**\n\n"
                        ...
                        f"• IFSC: `{ifsc_code}`\n\n"
                        f"📸 Scan QR or use UPI to pay.\n"
                        f"Then click **I have paid** to submit screenshot.",
                reply_markup=markup,
                parse_mode='Markdown'
            )
            self.bot.delete_message(call.message.chat.id, call.message.message_id)
            self.bot.answer_callback_query(call.id, f"✅ Plan selected: {plan['label']}")
        ...
        def paid_callback(call):
            ...
            self.bot.edit_message_caption(
                ...
                f"Plan: {plan['label']}\n"
                f"Amount: ₹{plan['price']}\n\n"
                f"Send a screenshot of your payment confirmation.\n"
                f"Also send UTR/Transaction ID in the caption.\n"
                f"Example: `UTR: 1234567890`",
                call.message.chat.id,
                call.message.message_id,
                parse_mode='Markdown'
            )
            self.bot.register_next_step_handler(call.message, process_payment_screenshot, plan)
        
        @self.bot.callback_query_handler(func=lambda call: call.data == 'cancel')
        def cancel_callback(call):
            self.bot.edit_message_text("❌ Cancelled.", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
        
        def process_payment_screenshot(message, plan):
            user_id = message.from_user.id
            ...
            try:
                ...
                payment_id = self.db.add_payment(user_id, plan['price'], plan_json, file_path, utr)
                self.db.add_log(user_id, 'payment_submitted', f'Payment ID: {payment_id}, UTR: {utr}, Plan: {plan["label"]}')
                ...
                if auto_approve == '1':
                    if self.db.approve_payment(payment_id):
                        ...
                        approval_msg = (
                            ...
                            f"⚠️ Valid for {plan['validity']} days"
                        )
                        markup = types.InlineKeyboardMarkup(row_width=1)
                        ...
                        self.bot.send_message(message.chat.id, "✅ Payment auto-approved! Credit added.", parse_mode='Markdown')
                        if self.admin_id:
                            self.bot.send_message(self.admin_id, f"✅ Auto approved\nUser: {message.from_user.first_name}\nID: {user_id}\nPlan: {plan['label']}\nAmount: ₹{plan['price']}\nUTR: {utr}", parse_mode='Markdown')
                        return
                
                # Manual
                self.bot.send_message(message.chat.id, f"✅ Payment submitted! ID: `{payment_id}`\n₹{plan['price']}\nUTR: `{utr or 'N/A'}`\n\n⏳ Waiting for admin.", parse_mode='Markdown')
                if self.admin_id:
                ...
                self.bot.send_message(message.chat.id, "❌ Failed to process. Try again.", parse_mode='Markdown')
        ...
        def go_deploy_callback(call):
            ...
            if credits <= 0:
                ...
                return
            self.bot.edit_message_text(f"🚀 **Deployment**\n\nStep 1: Send bot token.\n\n💳 You have {credits} credit(s).\n\nSend token from @BotFather (no spaces):", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
        ...
        def approve_payment(call):
            ...
            if self.db.approve_payment(payment_id):
                ...
                if payment:
                    ...
                    self.bot.send_message(call.message.chat.id, "✅ Approved! User notified.", parse_mode='Markdown')
                    self.db.add_log(user_id, 'payment_approved', f'Payment ID: {payment_id}')
                    self.bot.answer_callback_query(call.id, "✅ Approved!")
                else:
                    self.bot.answer_callback_query(call.id, "❌ Payment not found!", show_alert=True)
            else:
        ...
        def deploy_bot(message):
            ...
            credits = self.db.get_available_credits(user_id)
            if credits <= 0:
                self.bot.send_message(message.chat.id, f"❌ No credits. Buy credits first.", parse_mode='Markdown')
                return
        ...
        def process_bot_token(message):
            ...
            if not bot_token or ' ' in bot_token:
                self.bot.send_message(message.chat.id, "❌ Token contains spaces. Please send the token without spaces.", parse_mode='Markdown')
                return
            msg = self.bot.send_message(message.chat.id, "⏳ Validating token and deploying...", parse_mode='Markdown')
            ...
            if success:
                ...
                self.bot.send_message(message.chat.id, "🚀 Bot is now running!\n📦 Manage from My Deployments.", parse_mode='Markdown')
            else:
                self.bot.edit_message_text(f"❌ {result}", msg.chat.id, msg.message_id, parse_mode='Markdown')
        ...
        def my_deployments(message):
            ...
            deployments = self.db.get_user_deployments(user_id)
            if not deployments:
                self.bot.send_message(message.chat.id, "📦 No deployments.", parse_mode='Markdown')
                return
            ...
            if len(deployments) > 5:
                response += f"📊 Total: {len(deployments)}"
        ...
        def process_template_upload(m):
            ...
            try:
                ...
                with open(file_path, 'wb') as f:
                    f.write(downloaded)
                template_id = self.db.save_template(m.document.file_name, file_path, m.from_user.id)
                self.db.add_log(m.from_user.id, 'template_upload', f'ID: {template_id}')
        ...
        def payments_admin(m):
            ...
            if not payments:
                ...
                return
            response = f"💳 **Pending** ({len(payments)})\n\n"
            for p in payments[:10]:
                plan = json.loads(p['plan_json']) if p.get('plan_json') else {}
                label = plan.get('label', 'N/A')
                response += f"🆔 `{p['id']}`\n👤 {p['first_name']} (@{p['username'] or 'N/A'})\n📦 {label}\n💰 ₹{p['amount']}\n📝 UTR: `{p.get('utr', 'N/A')}`\n📅 {p['created_at']}\n───\n"
            self.bot.send_message(m.chat.id, response, parse_mode='Markdown')
        ...
        def deployments_admin(m):
            ...
            response = f"🤖 **All Deployments** ({len(deps)})\n\n"
            for d in deps[:10]:
        ...
        def upi_settings(m):
            ...
            acc = self.db.get_setting('account_number') or "Not Set"
            ifsc = self.db.get_setting('ifsc_code') or "Not Set"
            auto = self.db.get_setting('auto_approve') or '1'
            self.bot.send_message(m.chat.id,
                f"💰 **UPI Settings**\n\n🏦 UPI: `{upi_id}`\n📛 Name: {upi_name}\n🏛️ Bank: {bank_name}\n🔢 Account: `{acc}`\n📇 IFSC: `{ifsc}`\n🤖 Auto Approve: {'✅ ON' if auto == '1' else '❌ OFF'}",
                reply_markup=markup, parse_mode='Markdown')
        ...
        def add_credits_menu(m):
            ...
            markup.add(types.InlineKeyboardButton('👥 Select User', callback_data='add_credits_select_user'),
                       types.InlineKeyboardButton('🔙 Back', callback_data='back_admin'))
            self.bot.send_message(m.chat.id, "➕ **Add Credits Manually**\nSelect user:", reply_markup=markup, parse_mode='Markdown')
        ...
        def add_credits_select_user(call):
            ...
            users = self.db.get_all_users()
            if not users:
                self.bot.edit_message_text("❌ No users.", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
                return
            markup = types.InlineKeyboardMarkup(row_width=1)
            for u in users[:20]:
        ...
        def process_add_credits_amount(message, user_id):
            ...
            try:
                ...
                if count <= 0:
                    self.bot.send_message(message.chat.id, "❌ Positive number only.", parse_mode='Markdown')
                    return
                self.db.add_credits_manual(user_id, count, 0, 15)
                self.db.add_log(self.admin_id, 'manual_credit_add', f'Added {count} credits to {user_id}')
                user = self.db.get_user(user_id)
                if user:
                    ...
                    self.bot.send_message(user_id,
                        ...
                        parse_mode='Markdown')
                self.bot.send_message(message.chat.id, f"✅ Added {count} credit(s) to `{user_id}`.", parse_mode='Markdown')
                self.show_admin_menu(message)
            except:
                self.bot.send_message(message.chat.id, "❌ Invalid number.", parse_mode='Markdown')
        ...
        def plans_admin(m):
            ...
            if plans:
                for p in plans:
                    response += f"🆔 {p['id']} - {p['label']} - ₹{p['price']} - {p['validity']} days\n"
            else:
                response += "No plans.\n"
            self.bot.send_message(m.chat.id, response, reply_markup=markup, parse_mode='Markdown')
        
        @self.bot.callback_query_handler(func=lambda call: call.data == 'add_plan')
        def add_plan(call):
            if call.from_user.id != self.admin_id:
                self.bot.answer_callback_query(call.id, "⛔ Unauthorized!", show_alert=True)
                return
            self.bot.edit_message_text("➕ **Add Plan**\n\nSend in format:\n`label, price, validity`\nExample: `1 Credit - 30 Days, 199, 30`", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
            self.bot.register_next_step_handler(call.message, process_add_plan)
        ...
        def process_add_plan(message):
            ...
            try:
                ...
                if len(parts) != 3:
                    self.bot.send_message(message.chat.id, "❌ Use format: label, price, validity", parse_mode='Markdown')
                    return
                label = parts[0].strip()
                price = int(parts[1].strip())
                validity = int(parts[2].strip())
                if price <= 0 or validity <= 0:
                    self.bot.send_message(message.chat.id, "❌ Price and validity must be positive.", parse_mode='Markdown')
                    return
                plans = self.db.get_plans()
                ...
                self.bot.send_message(message.chat.id, f"✅ Plan added!\n{label} - ₹{price} - {validity} days", parse_mode='Markdown')
                self.show_admin_menu(message)
            except:
                self.bot.send_message(message.chat.id, "❌ Invalid format. Use: label, price, validity", parse_mode='Markdown')
        
        @self.bot.callback_query_handler(func=lambda call: call.data == 'delete_plan_list')
        def delete_plan_list(call):
            if call.from_user.id != self.admin_id:
                self.bot.answer_callback_query(call.id, "⛔ Unauthorized!", show_alert=True)
                return
            plans = self.db.get_plans()
            if not plans:
                self.bot.edit_message_text("❌ No plans to delete.", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
                return
            markup = types.InlineKeyboardMarkup(row_width=1)
            for p in plans:
                markup.add(types.InlineKeyboardButton(f"{p['label']} - ₹{p['price']}", callback_data=f"delete_plan_{p['id']}"))
            markup.add(types.InlineKeyboardButton('🔙 Back', callback_data='back_admin'))
            self.bot.edit_message_text("🗑️ Select plan to delete:", call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode='Markdown')
        
        @self.bot.callback_query_handler(func=lambda call: call.data.startswith('delete_plan_'))
        def delete_plan(call):
            ...
            self.bot.edit_message_text(f"✅ Plan deleted: {plan['label']}", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
            self.bot.answer_callback_query(call.id, f"✅ Deleted {plan['label']}")
        ...
        def edit_field(call):
            ...
            self.bot.edit_message_text(f"✏️ Send new {field.replace('_', ' ').title()}:", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
            self.bot.register_next_step_handler(call.message, process_edit, field)
        
        def process_edit(message, field):
            if message.from_user.id != self.admin_id: return
        ...
        def toggle_auto_approve(call):
            ...
            self.bot.edit_message_text(f"✅ Auto Approve {'ON' if new == '1' else 'OFF'}", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
        ...
        def change_welcome_image(call):
            if call.from_user.id != self.admin_id:
                self.bot.answer_callback_query(call.id, "⛔ Unauthorized!", show_alert=True)
                return
            self.bot.edit_message_text("🖼️ Send photo:", call.message.chat.id, call.message.message_id, parse_mode='Markdown')
            self.bot.register_next_step_handler(call.message, process_welcome_image)
        
        def process_welcome_image(message):
            if message.from_user.id != self.admin_id: return
            if not message.photo:
                self.bot.send_message(message.chat.id, "❌ Send photo.", parse_mode='Markdown')
                return
            try:
                file_info = self.bot.get_file(message.photo[-1].file_id)
                ...
                self.bot.send_message(message.chat.id, "❌ Error.", parse_mode='Markdown')
