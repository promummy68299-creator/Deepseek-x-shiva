...
# ==================== CONFIG ====================
BOT_TOKEN = "8831139428:AAEcYncFseyzZ86ybGrxS-pfOiKkx0rZN1k"
ADMIN_IDS = []
...
class Database:
    ...
    def add_plan(self, name, price, days, channel_link, description=''):
        ...
        conn.commit()
        conn.close()
        logger.info(f"✅ Plan added: {name} (ID: {plan_id})")
        return plan_id
    ...
    def approve_payment(self, payment_id):
        ...
        if payment:
            ...
            if plan:
                expiry = self.update_subscription(user_id, plan_id, plan['validity_days'])
                conn.commit()
                conn.close()
                return True, {"user_id": user_id, "plan": plan, "expiry": expiry}
    ...
    def reject_payment(self, payment_id, reason=""):
        ...
        if payment:
            user_id, plan_id = payment
            plan = self.get_plan(plan_id)
            return True, {"user_id": user_id, "plan": plan}
        return True, None
...
def refresh_payment_list(chat_id, message_id, admin_id):
    """Refresh the pending payments list after approve/reject"""
    try:
...
# Plans keyboard - Sirf name, price, validity
def plans_keyboard():
    plans = db.get_all_plans()
    logger.info(f"Building plans keyboard with {len(plans)} plans")
    kb = types.InlineKeyboardMarkup(row_width=1)
    for p in plans:
        label = f"{p['name']}  |  ₹{int(p['price'])} / {p['validity_days']}d"
        logger.info(f"Adding plan button: {label}")
...
def start_cmd(msg):
    ...
    if WELCOME_VIDEO:
        safe_video(user_id, WELCOME_VIDEO)
    elif WELCOME_IMAGE:
        safe_photo(user_id, WELCOME_IMAGE)
    ...
    if plans:
        ...
        safe_send(user_id, "❌ No plans available yet. Admin please add plans using /admin")
    ...
    try:
        ...
        # ========== PAY NOW ==========
            ...
            if plan:
                ...
                if qr:
                    safe_photo(user_id, qr, caption=text, reply_markup=payment_keyboard(plan_id))
                else:
                    safe_send(user_id, text + "\n\n⚠️ QR code not configured by admin.", 
            ...
            try:
                ...
                if payment:
                    ...
                    if user and plan:
                        ...
                        text += f"Your <b>{plan['name']}</b> subscription is now active!\n"
                        text += f"📅 Validity: {plan['validity_days']} days\n\n"
                        if link:
                            text += f"🔗 <a href='{link}'>Click Here to Access Channel</a>"
                        else:
