...
# ==================== CONFIG ====================
BOT_TOKEN = "8887238154:AAHLnHObGfq62FqxCN4TxG-0Js2MdNAjl5E"  # Apna token daalo
ADMIN_IDS = []  # Auto detect
...
def main_keyboard(user_id):
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("ℹ️ My Sub", callback_data="my_sub")
    )
    kb.add(
...
def plans_keyboard():
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    for p in plans:
        # Show price and days as per image style
...
def start_cmd(msg):
    ...
    if WELCOME_IMAGE:
        ...
        if WELCOME_IMAGE.startswith('video_'):
            video_id = WELCOME_IMAGE.replace('video_', '')
            safe_video(user_id, video_id, caption=text, reply_markup=main_keyboard(user_id))
        else:
            safe_photo(user_id, WELCOME_IMAGE, caption=text, reply_markup=main_keyboard(user_id))
    else:
    ...
    try:
        ...
        if data == "back_main":
            ...
            if user and user.get('subscription_plan_id'):
                ...
                if plan:
                    text = f"<b>ℹ️ Your Subscription</b>\n\n"
                    ...
                    if expiry and datetime.fromisoformat(expiry) > datetime.now():
                        ...
                        if plan.get('channel_link'):
                            text += f"🔗 <a href='{plan['channel_link']}'>Access Channel</a>"
                    else:
                        text += "❌ Status: <b>Expired</b>\n\nPlease purchase a new plan."
                    
                    kb = types.InlineKeyboardMarkup(row_width=1)
                    if not (expiry and datetime.fromisoformat(expiry) > datetime.now()):
                        kb.add(types.InlineKeyboardButton("🔄 Buy Plan", callback_data="show_plans"))
                    kb.add(types.InlineKeyboardButton("🔙 Back", callback_data="back_main"))
            ...
            text = "📖 <b>How to Use</b>\n\n1. Choose a plan from the list\n2. Click Buy Now\n3. Make payment via UPI/QR\n4. Send screenshot\n5. Admin approves\n6. Get channel access!"
            safe_edit(call.message.chat.id, call.message.message_id, text,
                     reply_markup=types.InlineKeyboardMarkup().add(
                     types.InlineKeyboardButton("🔙 Back", callback_data="back_main")))
        ...
        # ========== BUY PLAN ==========
            ...
            if plan:
                ...
                text += "\nAfter payment, click <b>I Paid</b> and send screenshot."
        ...
        # ========== ADMIN SETTINGS ==========
            ...
            if is_admin(user_id):
                ...
                safe_edit(call.message.chat.id, call.message.message_id,
                         "🖼️ Send new welcome image or video:",
                         reply_markup=types.InlineKeyboardMarkup().add(
...
def handle_video(msg):
    ...
    if user_id in user_data and 'setting' in user_data[user_id]:
        key = user_data[user_id]['setting']
        if key == 'welcome_image':
            db.set_setting('welcome_image', f'video_{file_id}')
            global WELCOME_IMAGE
            WELCOME_IMAGE = f'video_{file_id}'
