...
# ==================== CONFIG ====================
BOT_TOKEN = "8686350329:AAFqdL60KUocHOAvPA-FvNpHO2wIrzY0868"
CHECK_INTERVAL = 3600
...
TEMPLATE_REQ_PATH = os.path.join(BASE_DIR, "templates", "requirements.txt")
DEPLOYMENTS_PATH = os.path.join(BASE_DIR, "deployments")
LOG_PATH = os.path.join(BASE_DIR, "logs")
UPLOADS_PATH = os.path.join(BASE_DIR, "uploads")

for path in [os.path.dirname(DATABASE_PATH), "templates", DEPLOYMENTS_PATH, LOG_PATH, "payments", "settings", UPLOADS_PATH]:
...
class Database:
    ...
    def get_admin_id(self):
        val = self.get_setting('admin_user_id')
        return int(val) if val else None
    
    def set_admin_id(self, user_id):
        self.update_setting('admin_user_id', str(user_id))
    
    def get_requirements(self):
        return self.get_setting('requirements') or ''
    
    def set_requirements(self, content):
        self.update_setting('requirements', content)
...
class ZipHandler:
    @staticmethod
    def extract_and_save(zip_path: str, admin_id: int, db: Database) -> Tuple[bool, str]:
        try:
            extract_dir = os.path.join(UPLOADS_PATH, f"extract_{int(time.time())}")
            ...
            for root, dirs, files in os.walk(extract_dir):
                for file in files:
                    if file == 'bot.py':
                        bot_file = os.path.join(root, file)
                    elif file == 'requirements.txt':
                        req_file = os.path.join(root, file)
            
            if not bot_file:
                shutil.rmtree(extract_dir)
                return False, "❌ `bot.py` not found in zip."
            ...
            if 'YOUR_BOT_TOKEN' not in bot_content:
                shutil.rmtree(extract_dir)
                return False, "❌ `bot.py` must contain `YOUR_BOT_TOKEN` placeholder."
            
            # Save bot.py to templates
            template_path = os.path.join("templates", "bot.py")
            with open(template_path, 'w') as f:
                f.write(bot_content)
            
            # Save requirements if exists
            req_content = ""
            if req_file:
                with open(req_file, 'r') as f:
                    req_content = f.read()
                db.set_requirements(req_content)
                # Also save to templates folder
                shutil.copy(req_file, os.path.join("templates", "requirements.txt"))
            else:
                db.set_requirements("pyTelegramBotAPI==4.12.0\nrequests==2.31.0")
                with open(os.path.join("templates", "requirements.txt"), 'w') as f:
                    f.write("pyTelegramBotAPI==4.12.0\nrequests==2.31.0")
            
            # Save template to database
            db.save_template('bot.py', template_path, admin_id)
            db.add_log(admin_id, 'zip_upload', f'Uploaded zip with bot.py and requirements')
            
            # Cleanup
            shutil.rmtree(extract_dir)
            os.remove(zip_path)
            
            return True, f"✅ Template updated from zip!\n📄 `bot.py` saved\n📦 `requirements.txt` saved"
...
class DeploymentManager:
    ...
    def get_requirements(self) -> str:
        req = self.db.get_requirements()
        if req:
            return req
        return "pyTelegramBotAPI==4.12.0\nrequests==2.31.0"
    ...
    def deploy_bot(self, user_id: int, bot_token: str) -> Tuple[bool, str]:
        ...
        req_content = self.get_requirements()
        ...
        if success:
            ...
            if admin_id:
                try:
                    ...
                    telebot.TeleBot(BOT_TOKEN).send_message(admin_id, admin_msg, parse_mode='Markdown')
                except:
                    pass
    ...
    def start_bot(self, deploy_path: Path, bot_username: str) -> bool:
        try:
            ...
            with open(log_file, 'a') as f:
                ...
                f.write(f"Requirements: {req_file}\n\n")
            ...
            if req_file.exists():
                try:
                    subprocess.run([sys.executable, '-m', 'pip', 'install', '-r', str(req_file)], 
                                 cwd=str(deploy_path), capture_output=True, timeout=30)
                except:
                    pass
    ...
    def undeploy_bot(self, deployment_id: int) -> bool:
        deployment = None
        ...
        if not deployment:
            return False
        deploy_path = Path(deployment['deployment_path'])
        self.stop_bot(deploy_path)
        self.db.update_deployment_status(deployment_id, 'undeployed')
        self.db.add_log(deployment['user_id'], 'undeploy', f'Undeployed @{deployment["bot_username"]}')
        return True
...
class CreditExpiryChecker:
    ...
    def check(self):
        ...
        for credit in expired:
            ...
            if admin_id:
                try:
                    self.bot.send_message(admin_id, f"⏰ Credit expired\nUser: {user['first_name'] if user else 'Unknown'}\nID: {user_id}\nBots stopped: {len(deps)}", parse_mode='Markdown')
                except:
                    pass
...
class BotApplication:
    def __init__(self):
        ...
        self.admin_id = self.db.get_admin_id()
        if not self.admin_id:
            logging.info("No admin set yet. First user to start will become admin.")
        self.setup_handlers()
...
'''
        with open(p, 'w') as f:
            f.write(t)
        self.db.save_template('bot.py', str(p), self.admin_id if self.admin_id else 0)
        # Default requirements
        with open(os.path.join("templates", "requirements.txt"), 'w') as f:
            f.write("pyTelegramBotAPI==4.12.0\nrequests==2.31.0")
        self.db.set_requirements("pyTelegramBotAPI==4.12.0\nrequests==2.31.0")
    ...
    def setup_handlers(self):
        ...
        def start_cmd(m):
            ...
            admin_id = self.db.get_admin_id()
            if not admin_id:
                self.db.set_admin_id(uid)
                admin_id = uid
                self.admin_id = uid
                self.bot.send_message(uid, "👑 **You are now the admin!**\n\nYou have full control over the bot.\nUse the admin panel to manage everything.", parse_mode='Markdown')
                logging.info(f"First user {uid} set as admin.")
            ...
            try:
                ...
                if auto_approve == '1':
                    if self.db.approve_payment(pid):
                        ...
                        admin_id = self.db.get_admin_id()
                        if admin_id:
                            self.bot.send_message(admin_id, f"✅ Auto approved\nUser: {m.from_user.first_name}\nID: {uid}\nPlan: {plan['label']}\nUTR: {utr}", parse_mode='Markdown')
                        return
                # manual
                ...
                if admin_id:
                    ...
                    self.bot.send_photo(admin_id, open(file_path, 'rb'),
        ...
        def my_deployments(m):
            ...
            for i, d in enumerate(deps[:5], 1):
                ...
                if d['status'] == 'running':
                    resp += f"   [ 🗑️ Undeploy ](callback://undeploy_{d['id']})\n"
                resp += "\n"
            if len(deps) > 5:
            ...
            for d in deps[:5]:
                if d['status'] == 'running':
                    markup.add(types.InlineKeyboardButton(f"🗑️ Undeploy @{d['bot_username']}", callback_data=f"undeploy_{d['id']}"))
            if markup.keyboard:
        ...
        @self.bot.callback_query_handler(func=lambda c: c.data.startswith('undeploy_'))
        def undeploy_cb(c):
            uid = c.from_user.id
            ...
            if not dep:
                self.bot.answer_callback_query(c.id, "❌ Deployment not found!", show_alert=True)
                return
            if dep['status'] != 'running':
                self.bot.answer_callback_query(c.id, "⚠️ Bot is not running.", show_alert=True)
                return
            success = self.deployment_manager.undeploy_bot(dep_id)
            if success:
                self.bot.answer_callback_query(c.id, f"✅ Bot @{dep['bot_username']} undeployed!")
                self.bot.send_message(uid, f"✅ Bot @{dep['bot_username']} has been undeployed.\nIt is no longer running.", parse_mode='Markdown')
            else:
                self.bot.answer_callback_query(c.id, "❌ Failed to undeploy.", show_alert=True)
        ...
        def upload_template(m):
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(types.InlineKeyboardButton("📦 Upload Zip File", callback_data="upload_zip"))
            markup.add(types.InlineKeyboardButton("📄 Upload bot.py", callback_data="upload_single_py"))
            ...
            self.bot.send_message(
                ...
                "📦 **Zip File** – Contains `bot.py` + `requirements.txt`\n"
                "📄 **bot.py** – Single file (default requirements will be used)",
                reply_markup=markup,
                parse_mode='Markdown'
            )
        
        @self.bot.callback_query_handler(func=lambda c: c.data == 'upload_zip')
        def upload_zip_cb(c):
            admin_id = self.db.get_admin_id()
            if c.from_user.id != admin_id:
                self.bot.answer_callback_query(c.id, "⛔ Unauthorized!", show_alert=True)
            ...
            self.bot.edit_message_text(
                ...
                "📄 `bot.py` (must contain `YOUR_BOT_TOKEN`)\n"
                "📦 `requirements.txt` (optional)\n\n"
                "Click file icon and select your zip.",
                c.message.chat.id,
                c.message.message_id,
                parse_mode='Markdown'
            )
            self.bot.register_next_step_handler(c.message, process_zip_upload)
        
        def process_zip_upload(m):
            admin_id = self.db.get_admin_id()
            if m.from_user.id != admin_id:
                return
            
            if not m.document:
                self.bot.send_message(m.chat.id, "❌ Please send a file.", parse_mode='Markdown')
                self.bot.register_next_step_handler(m, process_zip_upload)
                return
            
            if not m.document.file_name.endswith('.zip'):
                self.bot.send_message(m.chat.id, "❌ Please send a `.zip` file.", parse_mode='Markdown')
                self.bot.register_next_step_handler(m, process_zip_upload)
                return
            ...
            try:
                ...
                zip_path = os.path.join(UPLOADS_PATH, f"{int(time.time())}_{m.document.file_name}")
                dl = self.bot.download_file(fi.file_path)
                with open(zip_path, 'wb') as f:
                    f.write(dl)
                
                # Process zip
                msg = self.bot.send_message(m.chat.id, "⏳ Processing zip file...", parse_mode='Markdown')
                success, result = ZipHandler.extract_and_save(zip_path, m.from_user.id, self.db)
                
                self.bot.edit_message_text(result, msg.chat.id, msg.message_id, parse_mode='Markdown')
                
            except Exception as e:
                self.bot.send_message(m.chat.id, f"❌ Error: {str(e)}", parse_mode='Markdown')
        
        @self.bot.callback_query_handler(func=lambda c: c.data == 'upload_single_py')
        def upload_single_py_cb(c):
            admin_id = self.db.get_admin_id()
            if c.from_user.id != admin_id:
                self.bot.answer_callback_query(c.id, "⛔ Unauthorized!", show_alert=True)
                return
            ...
            self.bot.edit_message_text(
                "📄 **Upload bot.py**\n\n"
                "Send the `bot.py` file.\n"
                "Make sure it contains `YOUR_BOT_TOKEN` placeholder.",
                c.message.chat.id,
                c.message.message_id,
                parse_mode='Markdown'
            )
            self.bot.register_next_step_handler(c.message, process_single_py_upload)
        
        def process_single_py_upload(m):
            admin_id = self.db.get_admin_id()
            if m.from_user.id != admin_id:
                return
            
            if not m.document or not m.document.file_name.endswith('.py'):
                self.bot.send_message(m.chat.id, "❌ Please send a `.py` file.", parse_mode='Markdown')
                self.bot.register_next_step_handler(m, process_single_py_upload)
                return
            
            try:
                fi = self.bot.get_file(m.document.file_id)
                file_path = os.path.join("templates", "bot.py")
                dl = self.bot.download_file(fi.file_path)
                content = dl.decode('utf-8')
                
                if 'YOUR_BOT_TOKEN' not in content:
                    self.bot.send_message(m.chat.id, "❌ Template must contain `YOUR_BOT_TOKEN` placeholder.", parse_mode='Markdown')
                    self.bot.register_next_step_handler(m, process_single_py_upload)
                    return
        ...
        def process_add_credits_amount(m, uid):
            ...
            try:
                ...
                self.db.add_log(admin_id, 'manual_credit_add', f'Added {count} to {uid}')
        ...
        def process_add_plan(m):
            ...
            try:
                ...
                self.db.add_log(admin_id, 'plan_added', f'Plan: {label}')
        ...
        def delete_plan(c):
            ...
            self.db.update_plans(plans)
            self.db.add_log(admin_id, 'plan_deleted', f'Plan: {plan["label"]}')
        ...
        def process_edit(m, field):
            ...
            self.db.add_log(admin_id, 'settings', f'Updated {field}')
        ...
        def toggle_auto_approve(c):
            ...
            self.db.add_log(admin_id, 'settings', f'Auto approve: {new}')
        ...
        def process_welcome_text(m):
            ...
            self.db.add_log(admin_id, 'settings', 'Welcome text updated')
        ...
        def process_welcome_image(m):
            ...
            try:
                ...
                self.db.add_log(admin_id, 'settings', 'Welcome image updated')
