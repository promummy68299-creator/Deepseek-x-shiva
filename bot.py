...
# ==================== CONFIG ====================
BOT_TOKEN = "8202779834:AAFhusnDMlazqVmUb7JWifbDz9Aj69DTO8M"
ADMIN_IDS = []
...
# Plans keyboard - 1 plan per line (NO EMOJI)
def plans_keyboard():
    plans = db.get_all_plans()
    kb = types.InlineKeyboardMarkup(row_width=1)
    for p in plans:
        label = f"{p['name']}  |  ₹{int(p['price'])} / {p['validity_days']} Days"
...
def start_cmd(msg):
    ...
    if WELCOME_VIDEO:
        safe_video(user_id, WELCOME_VIDEO, caption=f"<b>{BOT_NAME}</b>")
    elif WELCOME_IMAGE:
        safe_photo(user_id, WELCOME_IMAGE, caption=f"<b>{BOT_NAME}</b>")
    
    text = WELCOME_TEXT
    safe_send(user_id, text, reply_markup=main_keyboard(user_id))
    safe_send(user_id, "CHOOSE PLAN BELOW 😋")
    ...
    try:
        ...
        # ========== VIEW PLAN ==========
            ...
            if plan:
                ...
                text += "━━━━━━━━━━━━━━\n"
                text += f"🎬 Content approx: {len(media)} items\n"
            ...
            if is_admin(user_id):
                ...
                if payment:
                    ...
                    if user and plan:
                        ...
                        text += f"Your <b>{plan['name']}</b> subscription is active!\n"
                        text += f"📅 Validity: {plan['validity_days']} days\n\n"
                        if link:
...
def show_admin_payments(call):
    """Show pending payments after approve/reject"""
